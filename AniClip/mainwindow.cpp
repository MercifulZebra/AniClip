...
bool MainWindow::init(QString config_filename) {
    ...
    initSuccess_flag = initSuccess_flag && log->init(config_filename);
    ...
    if (initSuccess_flag) {
        ...
        initSuccess_flag = initSuccess_flag && centralStack->init(config_filename, log);
        ...
        qDebug() <<"Unable to initialize logger. Exiting";
    }
